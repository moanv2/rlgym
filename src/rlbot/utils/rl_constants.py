...
# ============================================================================
# Canonical aliases used by the reward stack (custom.py / custom_rl.py)
#
# These mirror the exact values in ``rlgym_sim.utils.common_values`` so the
# whole reward codebase can source field/physics constants from this one module
# (single source of truth) instead of importing some from rlgym_sim and some
# from here. Values are byte-checked equal to rlgym_sim — see test_rl_constants.
#
# NOTE the goal *back* (used as the shot/aim target) sits BEHIND the back wall
# at |y| = BACK_WALL_Y + GOAL_DEPTH = 6000 — it is NOT the goal centre on the
# wall (which is BLUE_GOAL_CENTER above at |y| = 5120). Keep them distinct.
# ============================================================================
BLUE_TEAM = 0
ORANGE_TEAM = 1

CAR_MAX_SPEED = CAR_MAX_SPEED_BOOSTING    # 2300.0 — rlgym_sim's CAR_MAX_SPEED

_GOAL_BACK_Y = BACK_WALL_Y + GOAL_DEPTH   # 6000.0 — goal mouth depth behind the wall
_GOAL_BACK_Z = GOAL_HEIGHT / 2            # 321.3875
BLUE_GOAL_BACK = (0.0, -_GOAL_BACK_Y, _GOAL_BACK_Z)     # (0, -6000, 321.3875)
ORANGE_GOAL_BACK = (0.0, _GOAL_BACK_Y, _GOAL_BACK_Z)    # (0,  6000, 321.3875)
