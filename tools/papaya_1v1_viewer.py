...
class AerialState(StateSetter):
    """Spawn the ball airborne with both cars fully boosted on the ground, facing it,
    so the bots must go up to challenge -> repeated aerials for cinematic footage."""

    def reset(self, state_wrapper):
        # Ball launched upward from low center so it arcs to ~1300uu and falls back,
        # giving both cars time to boost up and meet it in the air.
        bx = float(np.random.uniform(-700, 700))
        state_wrapper.ball.set_pos(bx, 0.0, 350.0)
        state_wrapper.ball.set_lin_vel(0.0, float(np.random.uniform(-300, 300)), 1450.0)
        state_wrapper.ball.set_ang_vel(0.0, 0.0, 0.0)
        for car in state_wrapper.cars:
            if car.team_num == 0:
                car.set_pos(0.0, -2000.0, 17.0)
...
def _latest_champion() -> str:
    """Auto-locate OUR live champion's latest COMPLETE checkpoint (2nd-newest, to skip a
    folder that may be mid-save), so --blue needs no manual path. Falls back to the published bot."""
    import glob
    cks = sorted(
        (Path(p) for p in glob.glob("checkpoints/exp_recipeH_distill-*/*") if Path(p).name.isdigit()),
    ...
    if cks:
        return str(cks[-1])
    return "martin-bots/checkpoints/CHAMPION_3.99B_advanced1024"  # published fallback (be on `martin`)


# --blue defaults to OUR live champion (auto-found); pass --orange for the opponent (e.g. Diego's v7).
DEFAULT_BLUE = _latest_champion()
...
def build_env(state_setter=None):
    """Minimal 1v1 AdvancedObs env. Defaults to a clean kickoff each episode; pass an
    AerialState() to spawn the ball airborne for aerial / cinematic footage."""
    import rlgym_sim
    ...
    return rlgym_sim.make(
        ...
        state_setter=state_setter if state_setter is not None else DefaultState(),
...
def run_match(
    ...
    headless without the rlviser binary open. Pass `state_setter=AerialState()` to
    spawn the ball airborne (aerial / cinematic footage).
    """
...
def main() -> None:
    ...
    p.add_argument(
        "--aerial",
        action="store_true",
        help="Spawn the ball airborne with both cars boosted so the bots aerial -- "
        "cinematic flying footage instead of a kickoff ground game.",
    )
    args = p.parse_args()
    ...
    run_match(
        ...
        state_setter=AerialState() if args.aerial else None,
    )
